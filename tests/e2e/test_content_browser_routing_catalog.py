...
class TestRoutingCatalog:
    ...
    def test_routing_catalog_contains_all_expected_keys(self, cb_custom_graph_page):
        ...
        for key in EXPECTED_ROUTING_KEYS:
            item = cb_custom_graph_page.locator(
                f'[data-testid="browser-routing-option-{key}"]'
            )
            assert item.count() == 1, (
                f"Routing option for key '{key}' not found in dropdown"
            )
    ...
    def test_round_segments_key_is_correct(self, cb_custom_graph_page):
        ...
        old = cb_custom_graph_page.locator(
            '[data-testid="browser-routing-option-round-seg"]'
        )
        assert old.count() == 0, (
            "'round-seg' key still present (not fixed to 'round-segments')"
        )
        new = cb_custom_graph_page.locator(
            '[data-testid="browser-routing-option-round-segments"]'
        )
    ...
    def test_taxi_is_present_in_catalog(self, cb_custom_graph_page):
        ...
        taxi = cb_custom_graph_page.locator(
            '[data-testid="browser-routing-option-taxi"]'
        )
    ...
    def test_cy_values_match_valid_cytoscape_curve_styles(self, cb_custom_graph_page):
        ...
        for key in EXPECTED_ROUTING_KEYS:
            ...
            assert current == key, (
                f"After _applyRouting('{key}'), _currentRouting = '{current}'"
            )
    ...
    def test_routing_dropdown_shows_all_catalog_entries(self, cb_custom_graph_page):
        ...
        btn.click()
        cb_custom_graph_page.wait_for_selector(
            '[data-testid="browser-routing-dropdown"]', timeout=5_000
        )
    ...
    def test_applying_taxi_routing_sets_curve_style_on_edges(
        self, cb_custom_graph_page
    ):
        """Selecting taxi routing applies curve-style:taxi to edges in cy."""
        cb_custom_graph_page.evaluate("() => _applyRouting('taxi')")
        cb_custom_graph_page.wait_for_function(
            "() => window.cy != null && window.cy.edges().length > 0"
        )
