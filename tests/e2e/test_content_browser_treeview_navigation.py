...
class TestTreeviewWorkflowClick:
    ...
    def test_workflow_row_click_expands_accordion_section(self, cb_graph_page):
        ...
        section_id = wf_row.locator(".browser-tree-toggle").get_attribute(
            "data-section"
        )
        ...
        assert display != "none", (
            f"Accordion section {section_id} not expanded after click"
        )
...
class TestTreeviewActivityClick:
    ...
    def test_activity_row_click_centers_camera_on_activity_node(self, cb_graph_page):
        ...
        act_row = cb_graph_page.locator(
            '[data-testid="browser-tree-row"][data-node-id^="activity:"]'
        ).first
        if act_row.count() == 0:
    ...
    def test_canvas_tap_activity_expands_parent_workflow_in_treeview(
        self, cb_graph_page
    ):
        ...
        row = cb_graph_page.locator(
            f'[data-testid="browser-tree-row"][data-node-id="{act_id}"]'
        )
    ...
    def test_canvas_tap_activity_highlights_row_in_treeview(self, cb_graph_page):
        ...
        if not act_nodes:
            pytest.skip("No activity nodes in graph")
        act_id = act_nodes[0]
        cb_graph_page.evaluate(f"() => _highlightTreeNode('{act_id}')")
        cb_graph_page.wait_for_timeout(300)
        row = cb_graph_page.locator(
            f'[data-testid="browser-tree-row"][data-node-id="{act_id}"]'
        )
