...
from e2e_helpers import login, open_content_browser, wait_for_cy_graph
...
def cb_playbook(cb_user, transactional_db):
    ...
    wf = Workflow.objects.create(name='CB Workflow', description='WF', playbook=pb, order=1)
    wf2 = Workflow.objects.create(name='CB Workflow 2', description='WF2', playbook=pb, order=2)
    ...
    act2 = Activity.objects.create(
        name='CB Implement', workflow=wf, order=2, predecessor=act1, agent=agent,
    )
