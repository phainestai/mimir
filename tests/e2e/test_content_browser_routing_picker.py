...
from e2e_helpers import open_content_browser, wait_for_cy_graph
...
class TestRoutingCatalog:
    ...
    def test_all_six_routing_options_present(self, cb_custom_graph_page: Page):
        ...
        for key in expected_keys:
            opt = cb_custom_graph_page.locator(f'[data-testid="browser-routing-option-{key}"]')
            expect(opt).to_be_visible()
        cb_custom_graph_page.keyboard.press('Escape')

    def test_default_routing_is_straight_in_custom_mode(self, cb_custom_graph_page: Page):
        """FOB-63 default routing is straight; button shows Straight in custom mode."""
        btn = cb_custom_graph_page.locator('[data-testid="browser-routing-btn"]')
        expect(btn).to_contain_text('Straight')

    def test_url_param_routing_sets_initial_state(self, cb_graph_page: Page, live_server, cb_playbook):
    ...
    def test_unknown_routing_param_falls_back_to_bezier(self, cb_graph_page: Page, live_server, cb_playbook):
...
class TestRoutingPickerInteraction:
    ...
    def test_click_opens_dropdown(self, cb_custom_graph_page: Page):
        ...
        cb_custom_graph_page.click('[data-testid="browser-routing-btn"]')
        dropdown = cb_custom_graph_page.locator('[data-testid="browser-routing-dropdown"]')
    ...
    def test_select_taxi_applies_curve_style_to_all_edges(self, cb_custom_graph_page: Page):
    ...
    def test_routing_change_does_not_rebuild_node_positions(self, cb_custom_graph_page: Page):
    ...
    def test_escape_closes_dropdown_without_changing_routing(self, cb_custom_graph_page: Page):
        ...
        cb_custom_graph_page.wait_for_selector('[data-testid="browser-routing-dropdown"]')
        cb_custom_graph_page.keyboard.press('Escape')
        expect(cb_custom_graph_page.locator('[data-testid="browser-routing-dropdown"]')).to_have_count(0)
    ...
    def test_outside_click_closes_dropdown(self, cb_custom_graph_page: Page):
        ...
        cb_custom_graph_page.click('[data-testid="browser-routing-btn"]')
        cb_custom_graph_page.wait_for_selector('[data-testid="browser-routing-dropdown"]')
        cb_custom_graph_page.click('[data-testid="browser-canvas"]', position={"x": 10, "y": 10})
        expect(cb_custom_graph_page.locator('[data-testid="browser-routing-dropdown"]')).to_have_count(0)

    def test_unbundled_bezier_option_present_in_dropdown(self, cb_custom_graph_page: Page):
        """Dropdown must include an unbundled-bezier option (S43 — FOB-35 fix)."""
        cb_custom_graph_page.click('[data-testid="browser-routing-btn"]')
        cb_custom_graph_page.wait_for_selector('[data-testid="browser-routing-dropdown"]')
        expect(cb_custom_graph_page.locator('[data-testid="browser-routing-option-unbundled-bezier"]')).to_be_visible()

    def test_selecting_unbundled_bezier_applies_and_stores_param(self, cb_custom_graph_page: Page):
        """Selecting unbundled-bezier updates button label and URL param (S43)."""
        cb_custom_graph_page.click('[data-testid="browser-routing-btn"]')
        cb_custom_graph_page.click('[data-testid="browser-routing-option-unbundled-bezier"]')
        expect(cb_custom_graph_page.locator('[data-testid="browser-routing-btn"]')).to_contain_text('Unbundled Bezier')
