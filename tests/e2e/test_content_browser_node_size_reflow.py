...
class TestNodeSizeReflow:
    ...
    def test_toggle_to_auto_width_triggers_layout_rerun(self, cb_graph_page):
        ...
        cb_graph_page.evaluate(
            "() => { if (window._nodeSizeMode !== 'fixed') _applyNodeSizeToggle(); }"
        )
        cb_graph_page.evaluate(
            "() => { if (window._nodeSizeMode !== 'fixed') _applyNodeSizeToggle(); }"
        )
    ...
    def test_toggle_to_fixed_size_triggers_layout_rerun(self, cb_graph_page):
        ...
        assert mode_before == mode_after, (
            "Mode did not return to original after two toggles"
        )
    ...
    def test_auto_width_nodes_width_reflects_label_length(self, cb_graph_page):
        ...
        cb_graph_page.evaluate(
            "() => { if (window._nodeSizeMode !== 'auto') _applyNodeSizeToggle(); }"
        )
        ...
        assert len(set(widths)) > 0, (
            "All activity widths are identical in auto-width mode"
        )
