...
def graph_playbook(transactional_db):
    ...
    user = User.objects.create_user(
        username="graph_e2e_user",
        email="graph_e2e@test.com",
    ...
    act2 = Activity.objects.create(
        name="Implement", workflow=wf, order=2, predecessor=act1, agent=agent
    )
...
def empty_graph_playbook(transactional_db):
    ...
    user = User.objects.create_user(
        username="empty_graph_user",
        email="empty_graph@test.com",
        password="testpass123",
    )
    mark_email_verified(user)
    pb = Playbook.objects.create(
...
def test_graph_renders_nodes_for_playbook_entities(
    ...
    has_workflow = page.evaluate(
        "() => window.cy.nodes('[type = \"workflow\"]').length > 0"
    )
    has_activity = page.evaluate(
        "() => window.cy.nodes('[type = \"activity\"]').length > 0"
    )
...
def test_graph_nodes_are_visually_differentiated(
    page: Page, live_server_url: str, graph_playbook
):
...
def test_contains_edge_connects_workflow_to_activity(
    page: Page, live_server_url: str, graph_playbook
):
    ...
    assert contains_count >= 2, (
        f"Expected at least 2 contains edges, got {contains_count}"
    )
...
def test_layout_workflow_above_activity(
    page: Page, live_server_url: str, graph_playbook
):
    ...
    login(page, live_server_url, data["username"], data["password"])
    page.goto(
        f"{live_server_url}/browser/{data['pb'].pk}/?layout=dagre-tb&compound=none"
    )
...
def test_loading_spinner_visible_during_fetch(
    page: Page, live_server_url: str, graph_playbook
):
...
def test_spinner_hidden_after_graph_renders(
    page: Page, live_server_url: str, graph_playbook
):
...
def test_empty_playbook_shows_no_content_state(
    page: Page, live_server_url: str, empty_graph_playbook
):
    ...
    assert (
        "no content"
        in page.get_by_test_id("browser-no-content-state").inner_text().lower()
    )


def test_empty_playbook_go_to_playbook_link(
    page: Page, live_server_url: str, empty_graph_playbook
):
...
def test_empty_playbook_does_not_render_cytoscape(
    page: Page, live_server_url: str, empty_graph_playbook
):
...
class TestNodeInsertionOrder:
    ...
    def test_activity_nodes_precede_resource_nodes(
        ...
        assert res_indices, (
            "No resource nodes found — check fixture has skills/rules/agent"
        )
    ...
    def test_resource_nodes_grouped_after_parent_activity(
        ...
        def resource_indices_for(act_pk):
            return [
                i for i, node_id in enumerate(order) if f":activity:{act_pk}" in node_id
            ]
    ...
    def test_order_preserved_after_filter_rebuild(
        ...
        act_indices = [i for i, t in enumerate(order) if t == "activity"]
        res_indices = [
            i for i, t in enumerate(order) if t not in ("workflow", "activity")
        ]
        assert wf_indices and act_indices, (
            "Missing workflow or activity nodes after rebuild"
        )
        assert max(wf_indices) < min(act_indices), (
            "Workflows not before activities after rebuild"
        )
        if res_indices:
            assert max(act_indices) < min(res_indices), (
                "Activities not before resources after rebuild"
            )
