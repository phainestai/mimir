...
def _open_resource_tree_page(page: Page, live_server_url: str, username: str, password: str, pk: int) -> None:
...
class TestResourceTreeRendering:
    ...
    def test_resource_tree_placeholder_on_load(self, page: Page, live_server, rt_playbook):
    ...
    def test_selecting_activity_shows_resource_tree(self, page: Page, live_server, rt_playbook):
        """Selecting an activity node renders the resource tree (FOB-28)."""
        _open_resource_tree_page(page, live_server.url, 'rt_user', 'testpass123', rt_playbook.pk)
    ...
    def test_resource_tree_shows_skill(self, page: Page, live_server, rt_playbook):
        ...
        _open_resource_tree_page(page, live_server.url, 'rt_user', 'testpass123', rt_playbook.pk)
        _tap_first_activity(page)
        expect(page.locator('[data-testid="browser-resource-tree"]')).to_contain_text('Shared Skill')

    def test_resource_tree_clears_on_panel_close(self, page: Page, live_server, rt_playbook):
        ...
        expect(page.locator('[data-testid="browser-resource-tree"]')).to_contain_text('Select a Workflow')

    def test_workflow_selection_aggregates_resources(self, page: Page, live_server, rt_playbook):
...
class TestResourceTreeClick:
    ...
    def test_clicking_resource_row_opens_detail_panel(self, page: Page, live_server, rt_playbook):
