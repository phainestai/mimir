...
def released_playbook(user, db):
    ...
    wf = Workflow.objects.create(
        name="BPE",
        description="Build Phase Execution",
        playbook=pb,
        order=1,
    )
...
class TestGraphAPIAccess:
    ...
    def test_team_member_can_access_team_playbook_graph(
        self, auth_client, team_playbook_access
    ):
    ...
    def test_team_playbook_appears_in_playbook_list_api(
        self, auth_client, team_playbook_access
    ):
...
class TestGraphAPIResponseShape:
    def test_graph_response_has_required_top_level_keys(
        self, auth_client, released_playbook
    ):
    ...
    def test_empty_playbook_returns_empty_nodes_and_edges(
        self, auth_client, empty_playbook
    ):
    ...
    def test_detail_url_and_embed_url_are_present(self, auth_client, released_playbook):
        ...
        for node in data["nodes"]:
            assert isinstance(node["detail_url"], str), (
                f"detail_url not str on {node['id']}"
            )
            assert len(node["detail_url"]) > 0, f"detail_url empty on {node['id']}"
            assert isinstance(node["embed_url"], str), (
                f"embed_url not str on {node['id']}"
            )
            # embed_url should append ?embed=1 to detail_url
            assert node["embed_url"].endswith("?embed=1"), (
                f"embed_url missing ?embed=1 on {node['id']}"
            )
...
class TestGraphAPIPhaseMetadata:
    def test_activity_with_phase_carries_phase_meta(
        self, auth_client, released_playbook
    ):
    ...
    def test_activity_without_phase_has_empty_meta(
        self, auth_client, released_playbook
    ):
...
class TestGraphAPIContainsEdges:
    def test_contains_edge_from_workflow_to_activity(
        self, auth_client, released_playbook
    ):
...
class TestGraphAPIResourceEdges:
    ...
    def test_agent_node_and_assigned_agent_edge_present(
        ...
        assert ":activity:" in agent_nodes[0]["id"], (
            "Agent node must be scoped per-activity"
        )
        agent_edges = [
            e for e in data["edges"] if e["relationship"] == "assigned_agent"
        ]
        assert len(agent_edges) == 1

    def test_rule_node_and_governed_by_rule_edge_present(
        self, auth_client, playbook_with_resources
    ):
        ...
        assert ":activity:" in rule_nodes[0]["id"], (
            "Rule node must be scoped per-activity"
        )
        rule_edges = [
            e for e in data["edges"] if e["relationship"] == "governed_by_rule"
        ]
        assert len(rule_edges) == 1

    def test_artifact_node_and_produces_edge_present(
        self, auth_client, playbook_with_resources
    ):
        ...
        assert ":activity:" in artifact_nodes[0]["id"], (
            "Artifact node must be scoped per-activity"
        )
    ...
    def test_same_resource_linked_to_two_activities_creates_two_nodes(
        self, user, auth_client, db
    ):
...
class TestGraphAPIDisplayCodeAndSequence:
    ...
    def ordered_playbook(self, user, db):
        ...
        wf = Workflow.objects.create(
            name="Build Phase", abbreviation="BPE", playbook=pb, order=1
        )
    ...
    def test_activity_node_includes_display_code(self, auth_client, ordered_playbook):
        ...
        data = auth_client.get(f"/api/playbooks/{pb.pk}/graph/").json()
        act_nodes = {
            n["entity_pk"]: n for n in data["nodes"] if n["type"] == "activity"
        }
    ...
    def test_activity_display_code_empty_when_no_abbreviation(
        self, user, auth_client, db
    ):
    ...
    def test_sequence_edges_not_cross_workflow(self, user, auth_client, db):
        ...
        cross = [
            e
            for e in seq_edges
            if e["source"] == f"activity:{act1.pk}"
            and e["target"] == f"activity:{act2.pk}"
        ]
        assert len(cross) == 0
